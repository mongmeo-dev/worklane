  ...
  <div class="fixed bottom-3 right-3 z-50 w-72 rounded-xl border bg-popover text-popover-foreground shadow-2xl">
    <div class="flex items-start gap-2.5 p-3.5">
      ...
      <div class="min-w-0 flex-1">
        <p class="text-[12.5px] font-semibold">새 버전 {updater.version}</p>
        <p class="mt-0.5 text-[10.5px] text-muted-foreground">업데이트를 설치하고 앱을 재시작합니다.</p>
        {#if updater.message}<p class="mt-1 text-[10px] text-destructive">{updater.message}</p>{/if}
      </div>
      {#if !downloading}
        <button type="button" class="grid size-6 shrink-0 place-items-center rounded-md text-muted-foreground hover:bg-accent hover:text-foreground" aria-label="나중에" onclick={() => updater.dismiss()}>
          <X class="size-3.5" />
        </button>
      {/if}
    </div>
    <div class="flex justify-end gap-1.5 border-t px-3.5 py-2.5">
      {#if !downloading}
        <button type="button" class="rounded-md border px-2.5 py-1 text-[11px] hover:bg-accent" onclick={() => updater.dismiss()}>나중에</button>
      {/if}
      <button
        type="button"
      ...
      >{downloading ? "설치 중…" : "설치 후 재시작"}</button>
    </div>
  </div>
{/if}
