...
<Dialog.Root bind:open>
  <Dialog.Content class="w-[860px] max-w-[calc(100%-2rem)] gap-0 overflow-hidden rounded-[14px] p-0 sm:max-w-[860px]">
    <Dialog.Header class="border-b px-5 py-3.5">
      <Dialog.Title class="text-base font-semibold">{t("taskBoard.title")}</Dialog.Title>
      <Dialog.Description class="text-xs">{t("taskBoard.desc")}</Dialog.Description>
    </Dialog.Header>

    <div class="flex flex-wrap items-center gap-1.5 border-b bg-muted/30 px-5 py-2.5">
      <Input class="h-8 min-w-40 flex-1 text-sm" bind:value={newTitle} placeholder={t("taskBoard.newTitle")} onkeydown={(e) => e.key === "Enter" && add()} />
      <Input class="h-8 min-w-40 flex-[2] text-sm" bind:value={newNotes} placeholder={t("taskBoard.newNotes")} />
      <select
    ...
    {#if error}
      <p class="border-b bg-destructive/10 px-5 py-1.5 text-2xs text-destructive">{error}</p>
    {/if}
    ...
    <div class="grid max-h-[460px] grid-cols-3 gap-3 overflow-auto p-4">
      {#each STATUSES as column (column.id)}
        <section class="flex min-h-40 flex-col gap-2 rounded-xl border bg-tile/50 p-2.5">
          <h2 class="flex items-center gap-1.5 px-1 text-xs font-semibold text-muted-foreground">
            {t(column.labelKey)}
            <span class="rounded-full bg-muted px-1.5 text-2xs">{byStatus(column.id).length}</span>
          </h2>
          {#each byStatus(column.id) as task (task.id)}
            <div class="rounded-lg border bg-card p-2.5">
              {#if editingId === task.id}
                <Input class="mb-1.5 h-7 text-sm" bind:value={editTitle} />
                <textarea class="mb-1.5 h-14 w-full resize-none rounded-md border bg-input/40 px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-ring" bind:value={editNotes}></textarea>
                <div class="flex justify-end gap-1.5">
                  <Button size="sm" variant="ghost" class="h-6 px-2 text-2xs" onclick={() => (editingId = null)}>{t("common.cancel")}</Button>
                  <Button size="sm" class="h-6 px-2 text-2xs" onclick={saveEdit} disabled={!editTitle.trim()}>{t("common.update")}</Button>
                </div>
              {:else}
                <div class="flex items-start gap-1.5">
                  <p class="min-w-0 flex-1 text-sm font-semibold">{task.title}</p>
                ...
                {#if projectName(task.projectId)}
                  <span class="mt-1 inline-block rounded-full bg-accent-share/10 px-1.5 py-0.5 text-2xs font-semibold text-accent-share">{projectName(task.projectId)}</span>
                {/if}
                {#if task.notes}
                  <p class="mt-1 line-clamp-2 whitespace-pre-wrap text-2xs text-muted-foreground">{task.notes}</p>
                {/if}
                <div class="mt-2 flex items-center gap-1">
                  ...
                  {#if task.status !== "done"}
                    <button type="button" class="ml-auto flex items-center gap-1 rounded-md bg-primary px-2 py-1 text-2xs font-semibold text-primary-foreground" onclick={() => start(task)}>
