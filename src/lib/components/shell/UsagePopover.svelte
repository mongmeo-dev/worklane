...
<div class="absolute bottom-[calc(100%+8px)] left-0 z-40 w-64 overflow-hidden rounded-xl border bg-popover text-popover-foreground shadow-2xl" role="dialog" aria-label={t("usagePopover.detailAria", { name: info.fullName })}>
  <header class="border-b px-3.5 py-3">
    <div class="flex items-center gap-2">
      ...
      <span class="rounded-full bg-muted px-2 py-0.5 text-[9px] font-semibold text-muted-foreground">{info.plan ?? (info.connected ? t("usagePopover.planUnknown") : t("usage.notConnected"))}</span>
    </div>
    <p class="mt-1 truncate font-mono text-[9.5px] text-muted-foreground">{info.account ?? t("usagePopover.accountUnknown")}</p>
  </header>
  ...
  <footer class="flex items-center gap-2 border-t px-3.5 py-2.5">
    <span class="text-[9.5px] text-muted-foreground">{info.tier ?? t("usagePopover.cliAccount")}</span>
