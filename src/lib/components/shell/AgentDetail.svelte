...
<section class="flex min-h-0 min-w-0 flex-1 flex-col gap-3.5 overflow-hidden px-[18px] pb-[18px] pt-[14px]">
  <!-- 좁은 폭에서도 한 줄을 유지한다. 제목/브랜치가 먼저 줄고, 액션 버튼은 줄바꿈 없이 고정.
       lastActivity처럼 다른 곳(사이드바·타일)에 이미 있는 정보는 폭이 부족하면 감춘다. -->
  <header class="flex shrink-0 items-center gap-2.5 whitespace-nowrap">
    <button type="button" class="grid size-7 shrink-0 place-items-center rounded-md text-muted-foreground transition-colors hover:bg-accent hover:text-foreground" aria-label={t("agentDetail.backOverview")} onclick={() => shell.goOverview()}>
      <ArrowLeft class="size-4" />
    </button>
    <StatusDot {status} size={10} />
    <h1 class="min-w-0 shrink truncate text-base font-bold">{agent.title}</h1>
    <span class="hidden min-w-0 shrink items-center gap-1 font-mono text-2xs text-muted-foreground lg:flex">
      <GitBranch class="size-3 shrink-0" /><span class="truncate">{agent.branch}</span>
    </span>
    {#if sharedAgents.length > 1}
      <span class="hidden shrink-0 rounded-full bg-accent-share/10 px-2 py-0.5 text-2xs font-semibold text-accent-share xl:inline">{t("agentDetail.sharedWorktree", { count: sharedAgents.length })}</span>
    {/if}
    <div class="ml-auto flex shrink-0 items-center gap-2">
      ...
      {#if group && group.members.length > 1}
        <button type="button" class="flex items-center gap-1 rounded-full border bg-card px-2.5 py-1 text-2xs font-semibold transition-colors hover:bg-accent" onclick={() => shell.openCompare(group.groupId)}>
          <GitFork class="size-3" />{t("agentDetail.compare", { count: group.members.length })}
        </button>
      {/if}
      <span class="hidden text-2xs text-muted-foreground 2xl:inline">{agent.lastActivity ?? t("common.waitingActivity")}</span>
      <StatusBadge {status} />
    </div>
  </header>
  ...
  {#if status === "blocked"}
    <div class="flex shrink-0 items-center gap-3 rounded-[11px] border border-status-blocked/30 bg-status-blocked/8 px-3.5 py-2.5">
      ...
      <div class="min-w-0 flex-1">
        <p class="text-sm font-semibold text-status-blocked-fg">{t("agentDetail.blockedTitle")}</p>
        <p class="mt-0.5 text-2xs text-muted-foreground">{t("agentDetail.blockedDesc")}</p>
      </div>
      <button type="button" class="rounded-full bg-status-blocked px-3 py-1.5 text-2xs font-bold text-status-blocked-on" onclick={() => shell.showTerminal()}>{t("agentDetail.goTerminal")}</button>
    </div>
  {/if}
  ...
  <div class="flex shrink-0 items-end gap-1 border-b">
    {#each terminals as term (term.id)}
      ...
      >
        <button
          type="button"
          class="flex items-center gap-1.5 py-1 pl-3 text-xs font-medium"
    ...
    <div class="relative">
      ...
      {#if pickerOpen}
        ...
        <div class="absolute left-0 top-full z-50 mt-1 min-w-44 rounded-lg border bg-card p-1 shadow-md">
          <p class="px-2 pb-1 pt-1 text-2xs font-semibold uppercase tracking-wide text-muted-foreground">{t("agentDetail.newTerminal")}</p>
          {#each agentKindStore.selectableKindIds as k (k)}
            <button type="button" class="flex w-full items-center rounded px-2 py-1.5 text-left text-xs hover:bg-accent" onclick={() => addTerminal(k)}>{agentKindStore.labelOf(k)}</button>
          {/each}
        </div>
      {/if}
    </div>
    {#if shell.openFilePath}
      <div class="flex items-center border-b-2 {shell.showEditor ? 'border-foreground text-foreground' : 'border-transparent text-muted-foreground'}">
        <button type="button" class="max-w-44 truncate py-1 pl-3 text-xs font-medium" onclick={() => shell.openFile(shell.openFilePath!)} oncontextmenu={fileTabContextMenu.oncontextmenu} onkeydown={fileTabContextMenu.onkeydown}>{shell.openFilePath.split("/").at(-1)}</button>
    ...
    <button
      ...
      class="border-b-2 px-3 pb-2 pt-1 text-xs font-medium {shell.showPreview ? 'border-foreground text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground'}"
    ...
    {:else if activeTerminal}
      {#key activeTerminal.id}
        <div class="flex h-full flex-col bg-terminal p-1.5">
          ...
          <div class="shrink-0 px-2 py-1 font-mono text-2xs text-white/60">{t("terminal.hint")}</div>
        </div>
      {/key}
    {/if}
  </div>
