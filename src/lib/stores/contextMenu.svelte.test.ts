...
describe("contextMenu store", () => {
  ...
  it("restores focus only for the current connected origin", async () => {
    ...
    const generation = openContextMenu({ ariaLabel: "Actions", items: [], point: { x: 0, y: 0 }, origin });

    closeContextMenu(generation);
    expect(focus).toHaveBeenCalledWith({ preventScroll: true });

    const disconnected = document.createElement("button");
    const disconnectedFocus = vi.spyOn(disconnected, "focus");
    const disconnectedGeneration = openContextMenu({ ariaLabel: "Actions", items: [], point: { x: 0, y: 0 }, origin: disconnected });
    closeContextMenu(disconnectedGeneration);
